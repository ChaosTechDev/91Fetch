...
from viewkey_batch.http import RateLimitedClient, _retry_after, build_client, load_cookies
...
@respx.mock
def test_build_client_warms_site_session_before_target_request():
    warmup = respx.get(url__regex=re.compile(r"https://example\.test/index\.php\?_vkb=\d+"))
    warmup.mock(return_value=httpx.Response(200, headers={"set-cookie": "CLIPSHARE=test; path=/"}))
    target = respx.get("https://example.test/list").mock(return_value=httpx.Response(200, text="ok"))

    with build_client(fast_config(), {}) as client:
        assert client.get("https://example.test/list").text == "ok"
