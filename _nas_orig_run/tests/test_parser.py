...
def test_listing_rejects_prefixed_decoy_with_different_viewkey():
    html = """
    <div class="well"><a href="/view_video.php?viewkey=wrong&c=atoken"><img src="wrong.jpg"><span class="video-title">Wrong</span></a></div>
    <div class="well"><a href="/view_video.php?viewkey=real&c=token"><img src="right.jpg"><span class="video-title">Right</span></a></div>
    """
...
def test_listing_cache_buster_preserves_category_query():
    result = fresh_listing_url("https://x.test/v.php?category=hot&page=2")
    assert "category=hot" in result
    assert "page=2" in result
    assert "_vkb=" in result


def test_first_listing_page_uses_canonical_url():
    url = "https://x.test/v.php?next=watch"
    assert listing_page_url(url, "page", 1) == url
    assert listing_page_url(url, "page", 2) == f"{url}&page=2"
