...
const state = {
  ...
  category: saved.category === "top_week" ? "top_month" : (saved.category || "latest"),
...
async function api(path, options = {}) {
  ...
  catch (_) { throw new Error(`服务器返回异常（HTTP ${response.status}）：${raw.slice(0, 160)}`); }
...
function filteredDownloads() {
  if (state.downloadFilter === "completed") return state.downloads.filter(item => item.state === "completed");
  if (state.downloadFilter === "failed") return state.downloads.filter(item => item.state === "failed");
  if (state.downloadFilter === "pending") return state.downloads.filter(item => ["queued", "downloading"].includes(item.state));
  return state.downloads;
}

function renderDownloads(totalSize = 0) {
  const visibleDownloads = filteredDownloads();
  ...
  $("#sidebarActiveCount").textContent = active;
  $("#sidebarCompletedCount").textContent = counts.completed;
  $("#sidebarFailedCount").textContent = counts.failed;
  ...
  if (signature !== state.downloadSignature) {
    ...
    <div class="download-row" data-key="${escapeHtml(item.viewkey)}" data-action-state="">
      ...
      <div class="row-status ${escapeHtml(item.state)}">
        ...
        <div class="row-error" title="${escapeHtml(item.error || "")}">${escapeHtml(item.error || "")}</div>
      </div>
  ...
  state.downloads.forEach(item => {
    ...
    const errorText = status.querySelector(".row-error");
...
async function loadDownloads() {
  try {
    ...
    data.downloads.filter(item => item.state === "completed").forEach(item => state.downloadedKeys.add(item.viewkey));
...
function setSettingFieldVisibility() {
  const daily = $("#settingScheduleMode").value === "daily";
  $("#dailyTimeField").classList.toggle("hidden", !daily);
  $("#intervalMinutesField").classList.toggle("hidden", daily);
  const inventoryDaily = $("#settingInventoryScheduleMode").value === "daily";
  $("#inventoryDailyField").classList.toggle("hidden", !inventoryDaily);
  $("#inventoryIntervalField").classList.toggle("hidden", inventoryDaily);
}

function renderSchedulerStatus(data) {
  const scheduler = data.scheduler || {};
  const parts = [scheduler.message || "定时任务未启用"];
  if (scheduler.next_run) parts.push(`下次运行：${scheduler.next_run.replace("T", " ")}`);
  $("#schedulerStatus").textContent = parts.join("　");
  const inventory = data.inventory_scheduler || {};
  const inventoryParts = [inventory.message || "库存定时刷新未启用"];
  if (inventory.last_run) inventoryParts.push(`上次：${inventory.last_run.replace("T", " ")}`);
  if (inventory.next_run) inventoryParts.push(`下次：${inventory.next_run.replace("T", " ")}`);
  if (inventory.file_count !== undefined) inventoryParts.push(`${inventory.file_count} 个文件 / ${inventory.duration || 0} 秒`);
  $("#inventorySchedulerStatus").textContent = inventoryParts.join("　");
}
...
function fillSettings(data) {
  ...
  $("#settingFilterDownloaded").checked = data.filter_downloaded;
  $("#settingAutoEnabled").checked = data.auto_download_enabled;
  $("#settingScheduleMode").value = data.schedule_mode;
  $("#settingIntervalMinutes").value = data.interval_minutes;
  $("#settingDailyTime").value = data.daily_time;
  $("#settingPages").value = data.pages_per_category;
  $("#settingUiRefresh").value = data.ui_refresh_seconds;
  $("#settingInventoryEnabled").checked = data.inventory_refresh_enabled;
  $("#settingInventoryScheduleMode").value = data.inventory_schedule_mode || "interval";
  $("#settingInventoryInterval").value = data.inventory_interval_minutes || 60;
  $("#settingInventoryDaily").value = data.inventory_daily_time || "04:00";
  document.querySelectorAll(".setting-category").forEach(input => { input.checked = data.categories.includes(input.value); });
...
async function refreshSchedulerStatus() {
  ...
  try { renderSchedulerStatus(await api("/api/settings")); } catch (_) {}
}
...
function collectSettings() {
  return {
    ...
    filter_downloaded: $("#settingFilterDownloaded").checked,
    auto_download_enabled: $("#settingAutoEnabled").checked,
    schedule_mode: $("#settingScheduleMode").value,
    interval_minutes: Number($("#settingIntervalMinutes").value),
    daily_time: $("#settingDailyTime").value,
    categories: [...document.querySelectorAll(".setting-category:checked")].map(input => input.value),
    pages_per_category: Number($("#settingPages").value),
    ui_refresh_seconds: Number($("#settingUiRefresh").value),
    inventory_refresh_enabled: $("#settingInventoryEnabled").checked,
    inventory_schedule_mode: $("#settingInventoryScheduleMode").value,
    inventory_interval_minutes: Number($("#settingInventoryInterval").value),
    inventory_daily_time: $("#settingInventoryDaily").value,
  };
}
...
function setView(view, {sync = true, replace = false} = {}) {
  ...
  $("#catalogSidebarContext").classList.toggle("hidden", view !== "catalog");
  $("#downloadSidebarContext").classList.toggle("hidden", view !== "downloads");
  $("#settingsSidebarContext").classList.toggle("hidden", view !== "settings");
  $("#refreshButton").title = view === "catalog" ? "刷新视频目录" : view === "downloads" ? "刷新下载任务" : "刷新设置";
  ...
  $("#viewSubtitle").innerHTML = view === "catalog"
    ...
    : view === "downloads" ? `${state.downloads.length} 个下载任务` : "下载、库存、界面与账号设置";
...
function applyControlState() {
  ...
  $("#catalogSidebarContext").classList.toggle("hidden", state.activeView !== "catalog");
  $("#downloadSidebarContext").classList.toggle("hidden", state.activeView !== "downloads");
  $("#settingsSidebarContext").classList.toggle("hidden", state.activeView !== "settings");
}
...
async function refreshActiveView() {
  if (state.activeView === "catalog") {
    await loadCatalog();
    await loadDownloads();
  } else if (state.activeView === "downloads") {
    await loadDownloads();
  } else {
    await loadSettings();
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("#settingScheduleMode").addEventListener("change", setSettingFieldVisibility);
  $("#settingInventoryScheduleMode").addEventListener("change", setSettingFieldVisibility);
  $("#loginForm").addEventListener("submit", async event => {
    ...
    try {
      ...
      state.settingsPollTimer = setInterval(refreshSchedulerStatus, 5000);
      await loadCatalog();
      await loadDownloads();
  ...
  $("#settingsForm").addEventListener("submit", async event => {
    ...
    if (!payload.categories.length) return toast("至少选择一个自动下载分类", true);
    try {
  ...
  $("#runNowButton").addEventListener("click", async () => {
    try {
      await api("/api/settings/run-now", {method: "POST", body: "{}"});
      toast("已开始立即刷新");
      await loadSettings();
    } catch (error) { toast(error.message, true); }
  });
  $("#runInventoryNowButton").addEventListener("click", async () => {
    try {
      await api("/api/settings/inventory/run-now", {method: "POST", body: "{}"});
      toast("已开始刷新本地库存");
      setTimeout(loadSettings, 600);
    } catch (error) { toast(error.message, true); }
  });
  $("#selectAllDownloads").addEventListener("change", event => {
    filteredDownloads().forEach(item => event.target.checked
  ...
  document.querySelectorAll(".settings-jump").forEach(button => button.addEventListener("click", () => {
    document.querySelectorAll(".settings-jump").forEach(item => item.classList.toggle("active", item === button));
    document.querySelectorAll("#settingsForm > .settings-section").forEach(section => section.classList.toggle("hidden", section.id !== button.dataset.target));
    $("#runNowButton").classList.toggle("hidden", button.dataset.target !== "automationSettings");
    $("#settingsView").scrollTop = 0;
  }));
  $("#settingsSearch").addEventListener("input", event => {
    const query = event.target.value.trim().toLowerCase();
    document.querySelectorAll(".settings-jump").forEach(button => {
  ...
  // Keep all controls usable while the initial listing request is still loading.
  void (async () => {
    try {
      const config = await api("/api/config");
      $("#siteHost").textContent = new URL(config.base_url).host;
